...
function creategraph(dice, sides, targetnums) {
	...
	for (k = 0; k < dice.length; k++) {
		...
		if (k < myChart.data.datasets.length) {
			...
			myChart.data.datasets[k].backgroundColor = context => {
				var index = context.dataIndex;
				var value = context.dataset.data[index].x;
				return value < targetnumcurrent ? colorBgArray[loopval % 7] : "rgba(74, 231, 24, 0.3)";
			};
			myChart.data.datasets[k].borderColor = context => {
				var index = context.dataIndex;
				var value = context.dataset.data[index].x;
